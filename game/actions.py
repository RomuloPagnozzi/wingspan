...
_ACTION_GENERATORS[GamePhase.EXTRA_FOOD_ACTION] = lambda _: [
    SimpleAction("trade_bird"),
    SimpleAction("skip_trade"),
]
_ACTION_GENERATORS[GamePhase.EXTRA_LAY_EGGS_ACTION] = lambda _: [
    SimpleAction("trade_food"),
    SimpleAction("skip_trade"),
]
_ACTION_GENERATORS[GamePhase.EXTRA_CARD_DRAW_ACTION] = lambda _: [
    SimpleAction("trade_egg"),
    SimpleAction("skip_trade"),
]
_ACTION_GENERATORS[GamePhase.COLLECT_FOOD] = get_collect_food_actions
_ACTION_GENERATORS[GamePhase.GAME_OVER] = lambda _: []
...
def _get_game_setup_actions(state: GameState) -> list[Action]:
    """Return setup actions."""
    current_player = state.players[state.current_player_index]

    if current_player.action_cubes == 9:
        return [SimpleAction("start_setup")]

    return [SimpleAction("end_setup")]


@action_generator(GamePhase.SELECT_INITIAL_CARDS)
def _get_select_initial_cards_actions(state: GameState) -> list[Action]:
    ...
    return [
        SelectInitialAction(tuple(comb["kept_birds"]), comb["kept_bonus"])
        for comb in combinations
    ]


@action_generator(GamePhase.DISCARD_FOOD)
def _get_discard_food_actions(state: GameState) -> list[Action]:
    ...
    return [
        FoodMapAction("discard_food_combo", frozen_map(comb)) for comb in combinations
    ]


@action_generator(GamePhase.MAIN_TURN)
def _get_main_turn_actions(state: GameState) -> list[Action]:
    ...
    if can_play_a_bird(current_player):
        actions.append(SimpleAction("play_bird"))

    actions.append(SimpleAction("gain_food"))

    total_available = get_available_bird_cards(state) + len(state.bird_tray)
    if total_available > 0:
        actions.append(SimpleAction("draw_cards"))

    return actions


@action_generator(GamePhase.END_TURN)
def _get_end_turn_actions(state: GameState) -> list[Action]:
    ...
    if execution and execution.phase == "end_turn_discard":
        ...
        return [IdAction("discard_card", bird_id) for bird_id in player.bird_hand]

    return []


@action_generator(GamePhase.LAY_EGGS)
def _get_lay_eggs_actions(state: GameState) -> list[Action]:
    ...
    return [EggMapAction("lay_eggs", frozen_map(comb)) for comb in combinations]


@action_generator(GamePhase.DRAW_CARDS)
def _get_draw_cards_actions(state: GameState) -> list[Action]:
    ...
    return [
        DrawCardsAction(tuple(comb["tray_birds"]), comb["deck_cards"])
        for comb in combinations
    ]


@action_generator(GamePhase.SELECT_BIRD_TO_DISCARD)
def _get_bird_discard_actions(state: GameState) -> list[Action]:
    """Return birds that can be discarded for extra food."""
    current_player = state.players[state.current_player_index]
    return [IdAction("discard_bird", bird_id) for bird_id in current_player.bird_hand]


@action_generator(GamePhase.SELECT_FOOD_TO_DISCARD)
def _get_food_discard_actions(state: GameState) -> list[Action]:
    """Return foods that can be discarded for extra egg."""
    current_player = state.players[state.current_player_index]
    return [NameAction("discard_food", food) for food in current_player.food]


@action_generator(GamePhase.SELECT_EGG_TO_DISCARD)
def _get_egg_discard_actions(state: GameState) -> list[Action]:
    ...
    for row in current_player.board:
        for spot in row:
            if spot.bird is not None and spot.bird.state.eggs > 0:
                actions.append(IdAction("discard_egg", spot.bird.id))

    return actions


@action_generator(GamePhase.PLAY_BIRD)
def _get_play_bird_actions(state: GameState) -> list[Action]:
...
def _get_pay_egg_cost_actions(state: GameState) -> list[Action]:
    ...
    return [EggMapAction("pay_eggs", frozen_map(comb)) for comb in combinations]


@action_generator(GamePhase.PAY_FOOD_COST)
def _get_pay_food_cost_actions(state: GameState) -> list[Action]:
    ...
    return [FoodMapAction("pay_food", frozen_map(comb)) for comb in combinations]


@action_generator(GamePhase.ACTIVATE_POWERS)
def _get_activate_powers_actions(state: GameState) -> list[Action]:
    ...
    if not stack:
        ...
        actions: list[Action] = [SimpleAction("skip_power")]
        ...
        if can_execute_power(state, power_entry):
            actions.append(SimpleAction("activate_power"))
