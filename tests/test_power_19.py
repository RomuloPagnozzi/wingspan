...
from game.engine import transition_state, _execute_power_19
...
class TestPower19Execution:
    ...
    def test_power_19_caches_food_on_bird(self):
        ...
        state = _execute_power_19(state, power_entry)

        assert spot.bird.stashed_food == initial_stashed + 1

    def test_power_19_multiple_activations_stack(self):
        """Multiple activations accumulate stashed food."""
        state = GameState()
        state.players = [Player(1), Player(2)]
        state.game_phase = GamePhase.ACTIVATE_POWERS
        ...
        assert spot.bird.stashed_food == 3
...
class TestPower19TriggerMatching:
    ...
    def test_power_19_no_trigger_on_fish(self):
        """Power 19 does NOT trigger when opponent gains fish."""
        state = GameState()
        state.players = [Player(1), Player(2)]
        ...
        with patch("game.utils.get_bird_power", return_value=mock_power):
            triggered = get_triggered_pink_powers(
                ...
                PinkTrigger.GAIN_FOOD,
                triggering_player_index=0,
                context={"food_type": "fish"},
            )

        assert len(triggered) == 0

    def test_power_19_no_trigger_on_fruit(self):
        """Power 19 does NOT trigger when opponent gains fruit."""
        state = GameState()
        state.players = [Player(1), Player(2)]
...
class TestPower19Validation:
    ...
    def test_power_19_skip_power(self):
        ...
        assert spot.bird.stashed_food == initial_stashed
